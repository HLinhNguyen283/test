...
// std::map<int, std::vector<int>> checkLog;
...
// //    // Start the thread to simulate and send sensor data
// //     std::thread checkLogThread(checkLogSensorMessage);
...
//         SensorMessage msg;
//         {
//             int sensorID = sensorDist(generator);
//             int value = valueDist(generator);
//             msg.sensorName = DEF_SENSOR_NAME + std::to_string(sensorID);
//             msg.sensorValue = value;
//             checkLog[sensorID].push_back(value);
//         }
...
// // {
// //     std::unique_lock<std::mutex> lock(mutex);
// //      // Wait until 'terminate' becomes true
// //     queueCondition.wait(lock, [] { return shouldTerminate.load(); });
// //     for(auto sensorID = checkLog.begin(); sensorID != checkLog.end(); ++sensorID)
// //     {
// //         std::cout << DEF_SENSOR_NAME << sensorID->first << "sent message: ";
// //         for(auto &val: sensorID->second) {
// //             std::cout << val << " ";
// //         }
// //         std::cout << std::endl;
// //     }
...
void handleSensorMessage()
{
    while (true) 
    {
        ...
        // If the message queue is empty and the simulation is completed, exit the loop
        if (messageQueue.empty() && shouldTerminate) {
            lock.unlock();
            threadLimitSemaphore.release();
            break;
        }
...
void simulateAndSendSensorMessage()
{
    ...
    for (int count = 0; count < DEF_SIMULATION_MAX_COUNT; ++count)
    {
...

        {
            ...
            // Check if the simulation should terminate
            if (count + 1 == DEF_SIMULATION_MAX_COUNT && messageQueue.empty() ) {
                // Set the flag to terminate the handling threads
                shouldTerminate.store(true);
            }
