...
// 試験用拡張
// --------------------------------------------------------------------------
CString CGetIniFileParam::GetReturnCode()
{
	TCHAR	tcWork[256];
	GetPrivateProfileString(D_SECTION_Responce, _T("ReturnCode"), _T("0"), tcWork, sizeof(tcWork), CCommon::GetToolIniPath());
	return CString(tcWork);
}

CString CGetIniFileParam::GetChannelStatus()
{
	TCHAR	tcWork[256];
	GetPrivateProfileString(D_SECTION_Responce, _T("ChannelStatus"), _T("ST"), tcWork, sizeof(tcWork), CCommon::GetToolIniPath());
	return CString(tcWork);
}

CString CGetIniFileParam::GetChannelPaper()
{
	TCHAR	tcWork[256];
	GetPrivateProfileString(D_SECTION_Responce, _T("ChannelPaper"), _T("PA"), tcWork, sizeof(tcWork), CCommon::GetToolIniPath());
	return CString(tcWork);
}
