...
void CWriteDebugLog::Initialize( const CString& inFileName )
{
	...
	m_Level = iniUnwinderManager_work.getDebugLogLevel();
...
**/
void CWriteDebugLog::InitializeXml( const CString& inFileName )
{
	USES_CONVERSION;				// A2W使用の場合の宣言
	...
	// ログ出力ファイル
	CString OutputFolder = CCommon::GetModuleDirectory() + "\\Log\\UW_CONNECT\\" + CGetTime::GetStrDate() + ".xml";
	if ( !::PathFileExists( OutputFolder ) )
	{	// 存在しない場合はフォルダを作成する
		::SHCreateDirectory( NULL, A2W(OutputFolder.GetString()) );
	}
