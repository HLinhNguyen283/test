...
**/
CXmlCompose::CXmlCompose(UwXjmfDataMap& inUwXjmfDataMap)
...
BOOL CXmlCompose::CreateCommandResource()
{
	...
	// UWTension
	//
	std::string UWTension = m_UwXjmfDataMap["UWTension"];
	bstr = UWTension.c_str();
	val = bstr;
	pElementMedia->setAttribute( L"scr:UWTension", val );

	//
	// MaxRunSpeed
	//
	std::string MaxRunSpeed = m_UwXjmfDataMap["MaxRunSpeed"];
	bstr = MaxRunSpeed.c_str();
	val = bstr;
	pElementMedia->setAttribute( L"scr:MaxRunSpeed", val );
...
{
	std::string strSavePath = CXmlLogData::GetSendLogFilePath();
