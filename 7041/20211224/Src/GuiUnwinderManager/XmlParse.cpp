...
**/
CXmlParse::CXmlParse(UwXjmfDataMap& outUwXjmfDataMap)
				...
				if ( pNode_Resource->QueryInterface( IID_IXMLDOMElement, (void**)&pElement_Resource ) == S_OK )
				{
					...
					if ( pNode_Resource->selectSingleNode(L"Media", &pNode_Media) == S_OK )
					{
						...
						if ( pNode_Media->QueryInterface( IID_IXMLDOMElement, (void**)&pElement_Media ) == S_OK )
						{
							...
							pElement_Media->getAttribute(L"RollDiameter",&val);
							if ( val.vt != VT_NULL )
							{
								bstr = val;
...
**/
CXmlParseFile::CXmlParseFile(const std::string& inXmlFilePath, UwXjmfDataMap& outUwXjmfDataMap)
...
/**
	...
	* @param  outUwXjmfDataMap : [o]XMLデータから取り出した情報
**/
CXmlParseData::CXmlParseData(const std::string& inXmlData, UwXjmfDataMap& outUwXjmfDataMap)
	: CXmlParse(outUwXjmfDataMap)
	, m_XmlData(inXmlData)
{
}
