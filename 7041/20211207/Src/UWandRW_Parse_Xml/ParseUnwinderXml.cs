...
namespace UWandRW_Parse_Xml
{
	class ParseXML
	{
        ...
        private string parseSignalStatus(XmlNode inSignalStatus, XmlNamespaceManager inNsmgr)
        {
            ...
            {
                param += "SubType=Status ";
                param += "Status=";
                param += getAttributeValue(nodeDeviceInfo, "Status");
                param += " ";
            }
            // ResourceInfo/ResourceSet/Resource
            XmlNode nodeResource = inSignalStatus.SelectSingleNode("a:ResourceInfo/a:ResourceSet/a:Resource", inNsmgr);
            if (null != nodeResource)
            {
                param += "SubType=Paper ";
                param += "DescriptiveName=";
                param += getAttributeValue(nodeResource, "DescriptiveName");
                param += " ";
            }
