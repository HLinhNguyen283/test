...
public:
	...
	**/
	static std::string GetSendLogFilePath();

	/**
		* @brief  XMLレスポンスログファイルパス取得
		* @retval XMLレスポンスログファイルパス
	**/
	static std::string GetResponseLogFilePath();
...
protected:
	...
	**/
	static std::string GetLogFolderPath();
	...
	**/
	static std::string GetLogFilePath(const std::string& inName);
