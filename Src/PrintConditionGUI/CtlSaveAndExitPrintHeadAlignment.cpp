/*! \file CtlSaveAndExitPrintHeadAlignment.cpp
...
/*! \brief constructor
*/
CCtlSaveAndExitPrintHeadAlignment::CCtlSaveAndExitPrintHeadAlignment()
{
...
*/
void CCtlSaveAndExitPrintHeadAlignment::OnSetAttribute()
{
...
*/
long CCtlSaveAndExitPrintHeadAlignment::OnCommand(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case BN_CLICKED:
		{
			...
			if(ctlWnd == m_ctl[CTRLID_BN_SAVE]){
				...
				if(pData->SaveAddPrintHeadAlignmentDlg()){
					// close dialog
			...
			// cancel button
				...
				if(pData->CancelAddPrintHeadAlignmentDlg()){
					// close dialog
					return DEF_EXIT_PAGE;
				}
			}
...
*/
void CCtlSaveAndExitPrintHeadAlignment::OnUpdateState()
{
	if(m_ctl){
		...
		const char* strName = pData->GetAddPrintHeadAlignmentName_AddDlg();
		// the print head alignment name is non-input
		if(NULL == strName
			|| 0 == strlen(strName))
		{
			// disable the control
