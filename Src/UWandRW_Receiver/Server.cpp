...
**/
CServer::CServer(UINT InSendThreadID, CWriteDebugLog* InWriteDebugLog, const char* inDeviceName)
	: m_nSendThreadID(InSendThreadID), m_pWriteLog(InWriteDebugLog), m_DeviceName(inDeviceName)
{
}
...
DWORD CServer::HttpPostReceive(HANDLE hReqQueue, PHTTP_REQUEST pHttpRequest)
{
	...
	if (pHttpRequest->Flags & HTTP_REQUEST_FLAG_MORE_ENTITY_BODY_EXISTS)
	{
		...
		{	// 受信コマンドをログ出力する
			...
			sReceiveCommandLog.InitializeXml(FileName, m_DeviceName);
...
CString CServer::GetRecvStatusURL()
{
	...
	std::string IpAddress  = iniPrinterDescriptor.GetClientPcIPAddr(true);

	CIni_UwRwManager iniUwRwManager;
	iniUwRwManager.Initialize(TRUE);
	std::string ReceiverPort = iniUwRwManager.getReceiver_Port(m_DeviceName);
	std::string FolderName = iniUwRwManager.getStatus_Folder(m_DeviceName);
...
CString CServer::GetRecvPaperURL()
{
	...
	iniUwRwManager.Initialize(TRUE);
	std::string ReceiverPort = iniUwRwManager.getReceiver_Port(m_DeviceName);
	std::string FolderName = iniUwRwManager.getResource_Folder(m_DeviceName);
