...
**/
std::string CXmlLogData::GetSendLogFilePath(const char* inDeviceName)
{
	USES_CONVERSION;				// A2W使用の場合の宣言

	std::string LogFolderPath = GetLogFolderPath(inDeviceName);
	if ( !::PathFileExists( LogFolderPath.c_str() ) )
	{	// 存在しない場合はフォルダを作成する
		::SHCreateDirectory( NULL, A2W(LogFolderPath.c_str()) );
	}
	std::string LogFilePath = GetLogFilePath("Send", inDeviceName);
	if ( ::PathFileExists( LogFilePath.c_str() ) )
	{	// 存在する場合は１ミリ待って再度ファイルパスを作成する
		Sleep(1);
		LogFilePath = GetLogFilePath("Send", inDeviceName);
	}
	return LogFilePath;
}
...
**/
std::string CXmlLogData::GetResponseLogFilePath(const char* inDeviceName)
{
	USES_CONVERSION;				// A2W使用の場合の宣言
	...
	std::string LogFilePath = GetLogFilePath("Response", inDeviceName);
	if ( ::PathFileExists( LogFilePath.c_str() ) )
	{	// 存在する場合は１ミリ待って再度ファイルパスを作成する
		Sleep(1);
		LogFilePath = GetLogFilePath("Response", inDeviceName);
	}
	return LogFilePath;
}
...
**/
std::string CXmlLogData::GetLogFolderPath(const char* inDeviceName)
{
...
**/
std::string CXmlLogData::GetLogFilePath(const std::string& inName, const char* inDeviceName)
{
	std::string LogFilePath = GetLogFolderPath(inDeviceName);
	LogFilePath.append("\\");
